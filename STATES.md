...
**Regla explícita (definida 05-ago-2026, decisión del usuario tras evaluar alternativas — ajustada el mismo día tras probarla en vivo):** `FolderScanner` solo desciende **1 nivel de subcarpeta** por debajo de la carpeta que configuras para un sistema — la convención `<sistema>/<juego>/<archivo>` (un archivo suelto en la raíz, o dentro de exactamente una subcarpeta de juego). Si encuentra una subcarpeta más profunda, **se salta ese subárbol completo** (nunca enumera su contenido) y lo reporta en el panel de Log ("Skipped (nested too deep, not scanned): ...") — el resto de la carpeta configurada se sigue escaneando normalmente.

**Por qué (y por qué NO aborta todo el escaneo):** sin límite, apuntar la app a una carpeta demasiado amplia (la raíz del disco, la carpeta de usuario) intentaría enumerar recursivamente TODO lo que hay debajo — sin sentido y potencialmente muy lento. La primera versión de esta regla abortaba el escaneo COMPLETO de la carpeta en cuanto encontraba un subárbol demasiado profundo — corregido en vivo el mismo día tras probarlo: un solo subárbol problemático (ej. `BATOCERA`) volvía inutilizable el resto de una carpeta perfectamente escaneable. Ahora se salta únicamente ese subárbol, punto por punto, sin afectar nada más.

**Caso real que motivó esto:** la carpeta CPS3 real del usuario tiene una subcarpeta `BATOCERA` que duplica varios juegos un nivel más profundo de lo permitido (`CPS3/BATOCERA/sfiii3/cap-33s-1.chd` = 2 niveles, viola el límite de 1). Con la regla actual, `BATOCERA` simplemente se omite del escaneo (se loguea, nunca se enumera) — el resto de CPS3 (`sfiii`, `sfiii2`, `sfiii3`, `sfiii3ur1`) escanea con normalidad. De hecho, esa misma duplicación física era la causa raíz del bug de CHD duplicado corregido en §4e5 — al omitirse `BATOCERA` del escaneo, tampoco vuelve a aparecer esa duplicación.
