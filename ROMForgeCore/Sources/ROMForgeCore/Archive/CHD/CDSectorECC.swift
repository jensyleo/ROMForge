...
/// Reed-Solomon P/Q error-correction for Mode 1/2 CD-ROM sectors — ported
/// byte-for-byte from MAME's own `cdrom_file::ecc_verify`/`ecc_generate`/
/// `ecc_clear` (`src/lib/util/cdrom.cpp`, fetched 2026-07-30 from
/// github.com/mamedev/mame to get the exact lookup tables and algorithm
/// rather than reconstruct them from memory — a single wrong table entry
/// here would silently corrupt every "cdlz"/"cdzl"/"cdfl" CD-CHD hunk this
/// reconstructs).
///
/// CHD's CD-composite codecs (`cdlz`/`cdzl`/`cdfl`) strip a sector's own
/// 12-byte sync pattern and P/Q ECC bytes before compressing it, *when*
/// `ecc_verify` confirms they're exactly what `ecc_generate` would produce
/// (the overwhelmingly common case for real, valid CD dumps) — this makes
/// the sector far more compressible. Decompression must regenerate those
/// exact same bytes to reconstruct the original sector byte-for-byte, or
/// the file's hash won't match the DAT even though the actual game data
/// decoded perfectly.
public enum CDSectorECC {
    private static let syncOffset = 0x000
    private static let syncNumBytes = 12
    private static let modeOffset = 0x00f
    private static let eccPOffset = 0x81c
    private static let eccPNumBytes = 86
    private static let eccPComp = 24
    private static let eccQOffset = eccPOffset + 2 * eccPNumBytes
    private static let eccQNumBytes = 52
    private static let eccQComp = 43

private static let ecclow: [UInt8] = [
...
private static let ecchigh: [UInt8] = [
...
private static let poffsets: [[UInt16]] = [
...
private static let qoffsets: [[UInt16]] = [
    ...
    private static func eccSourceByte(_ sector: [UInt8], _ offset: Int) -> UInt8 {
        (sector[modeOffset] == 2 && offset < 4) ? 0x00 : sector[syncOffset + syncNumBytes + offset]
    }

    private static func eccComputeBytes(_ sector: [UInt8], _ row: [UInt16], _ rowlen: Int) -> (UInt8, UInt8) {
        var val1: UInt8 = 0
        var val2: UInt8 = 0
        for component in 0..<rowlen {
            let byte = eccSourceByte(sector, Int(row[component]))
            val1 ^= byte
            val2 ^= byte
            val1 = ecclow[Int(val1)]
        }
        val1 = ecchigh[Int(ecclow[Int(val1)] ^ val2)]
        val2 ^= val1
        return (val1, val2)
    }

    /// True only if every P and Q ECC byte in `sector` (2352-byte CD sector)
    /// exactly matches what `generate(...)` would compute for it — the
    /// precondition CHD's own encoder checks before it's allowed to strip
    /// the sync/ECC bytes for this sector.
    public static func verify(_ sector: [UInt8]) -> Bool {
        for byte in 0..<eccPNumBytes {
            let (val1, val2) = eccComputeBytes(sector, poffsets[byte], eccPComp)
            if sector[eccPOffset + byte] != val1 || sector[eccPOffset + eccPNumBytes + byte] != val2 {
                return false
            }
        }
        for byte in 0..<eccQNumBytes {
            let (val1, val2) = eccComputeBytes(sector, qoffsets[byte], eccQComp)
            if sector[eccQOffset + byte] != val1 || sector[eccQOffset + eccQNumBytes + byte] != val2 {
                return false
            }
        }
        return true
    ...
    /// Overwrites `sector`'s own P/Q ECC bytes with freshly computed ones —
    /// used to reconstruct a sector CHD stripped, after its 12-byte sync
    /// pattern has already been restored by the caller.
    public static func generate(_ sector: inout [UInt8]) {
        for byte in 0..<eccPNumBytes {
            let (val1, val2) = eccComputeBytes(sector, poffsets[byte], eccPComp)
            sector[eccPOffset + byte] = val1
            sector[eccPOffset + eccPNumBytes + byte] = val2
        }
        for byte in 0..<eccQNumBytes {
            let (val1, val2) = eccComputeBytes(sector, qoffsets[byte], eccQComp)
            sector[eccQOffset + byte] = val1
            sector[eccQOffset + eccQNumBytes + byte] = val2
        }
    }

    /// Zeroes a sector's own P/Q ECC bytes — mirrors MAME's `ecc_clear`,
    /// used by the encoder side; kept here for symmetry/testing even though
    /// decompression only ever calls `generate`.
    public static func clear(_ sector: inout [UInt8]) {
        for i in 0..<(2 * eccPNumBytes) { sector[eccPOffset + i] = 0 }
        for i in 0..<(2 * eccQNumBytes) { sector[eccQOffset + i] = 0 }
    }
}
