...
jensyleo's own second follow-up report (2026-08-26): still felt slow — the divider itself lagged visibly behind the mouse while actively dragging, on every divider in the app, not just the one this whole bug was chased through. That pointed at `splitViewDidResizeSubviews`, present unmodified since this file's very first commit: `NSSplitView` fires it on every single step of a live drag (not just once the user releases), and it was calling `UserDefaults.standard.set(_:forKey:)` synchronously on the main thread — the same thread driving the drag's own event-tracking loop — dozens of times a second for as long as the drag lasted. Debounced the same way as the restore above: only the actual write is deferred/coalesced (native divider tracking is entirely AppKit's own, untouched by this), so `UserDefaults` now stays untouched while a drag is in progress and only writes once motion actually stops for a beat — the very last position is still exactly what ends up persisted.
