...
A first fix re-applied the saved fractions on every width change until the same width was reported twice in a row, then locked. Still not enough — instrumented again, live: the transient ~868pt width itself got reported on two (or more) consecutive layout passes before the real growth to ~1438pt ever happened, so "the same width twice" isn't proof of real stability either, and the fix locked in on the still-transient width just as before.

Replaced with a plain debounce: every layout call reschedules the actual restore a short delay out, cancelling whatever was scheduled before — only once layout calls stop arriving for that whole stretch does the restore actually run, against whatever width is current by then. That can't land on a mid-sequence transient no matter how many times one repeats, since each repeat just pushes the deadline back out again. Verified directly, repeatedly, against a real installed build: forced a previously-corrupting saved fraction via the preference file, launched, waited, clicked around the games list, then quit and relaunched — the value now round-trips to 8 decimal places at every step instead of drifting toward the pane's minimum width.

jensyleo's own follow-up report (2026-08-26): resizing panels now felt slow. The first version of the debounce above used a 250ms delay — safely long enough to clear the transient layout passes, but paid unconditionally on *every* launch, for *every* nested split in this window (five of them), which read as the whole window's panels visibly, sluggishly snapping into place rather than simply appearing already correct (previously, restoration was immediate). Reduced to 60ms — the transient-to-final layout sequence itself resolves within a handful of AppKit layout passes well under that window in practice, so this still lands with ample margin over the actual settle time while falling under the range most people perceive as a deliberate delay.

### Known issue — dragging a split divider feels laggy (under investigation, cause not yet identified)

jensyleo's own report (2026-08-26): dragging any divider in the window makes it visibly lag behind the mouse. Two candidate causes were tried and both are now ruled out by direct measurement rather than reasoning:

- **Debounced `UserDefaults` writes during a drag.** `splitViewDidResizeSubviews` (unmodified since this file's first commit) writes on every step of a live drag, which looked like a plausible main-thread cost. Instrumented and timed it: the write path measures ~0.3ms per call. Debouncing it did not change the reported behavior, so the change was reverted rather than left in — it bought no measurable time and introduced a real edge case (a position left unsaved if the app quits inside the debounce window).
- **The restore step fighting the drag.** Logged every `setPosition` the restore path performs: across a full instrumented drag it fired zero times, so the saved-layout restore is not repositioning the divider mid-drag.

Profiling the app with `sample` during repeated pane resizes shows the process essentially idle (the overwhelming majority of samples parked in `mach_msg`/`__workq_kernreturn`, with negligible time in ROMForge's own frames). An earlier reading of ~180ms per SwiftUI body re-evaluation turned out to be an artifact of driving the resizes through `osascript`, whose own per-invocation overhead dominated the wall-clock gaps — not app work; that reading is withdrawn.

The remaining obstacle is reproduction: synthetically driven divider drags (via `cliclick`) do not reliably engage `NSSplitView`'s modal mouse-tracking loop, so the drag path that actually feels slow to a person has not yet been exercised under a profiler. Left explicitly open and unfixed rather than papered over with another speculative change.
