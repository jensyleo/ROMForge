        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 18) {
                ...
                helpSection("1G1R — \"Show Only 1G1R\"") {
                    Text("\"One Game, One ROM\": many MAME DATs list the same game several times over, once per regional release — a world version, a USA version, a Japan version, and so on, related to each other as a parent set and its clones. 1G1R is a purely visual feature for collections like that: it picks one preferred variant per family and can hide the rest, so the table reads as one row per game instead of three or four near-duplicates. It never deletes, moves, or renames any file — ROMForge stays read-only regardless of this setting.")
                    Text("Region detection reads the game's description straight out of the DAT, looking for a recognizable region name inside the parenthesized release tag most DATs already carry — e.g. \"Street Fighter II' (World 910522)\" is detected as World, \"Street Fighter II' (USA 910228)\" as USA. Nothing is guessed from the filename or the internal machine name, only from that description text.")
                    Text("Which region wins when a family has more than one recognizable variant is configured in Settings → View Options → \"1G1R region priority\": an ordered list (World, USA, Europe, Japan, etc. by default) where earlier beats later — drag entries to reorder. Change it any time; every open system re-evaluates its families against the new order immediately.")
                    Text("The star in the \"1G1R\" column (hidden by default — enable it from the column picker like any other hidden column) marks exactly one row per family: whichever variant wins under the current region priority. It shows up whether or not \"Show Only 1G1R\" is on — the star answers \"which one would be preferred,\" the toggle answers \"should the others be hidden right now,\" and the two are independent.")
                    Text("\"Show Only 1G1R\" itself, toggled in Settings → View Options → \"1G1R\", hides every non-preferred sibling in a family from the table, leaving just the starred row. The games list title reflects this with a \"· 1G1R hides n\" suffix showing how many rows are currently hidden by it.")
                    Text("A family where no variant's description names any region on the priority list is left alone either way: nothing gets a star, and \"Show Only 1G1R\" hides nothing in that family — with no reliable way to rank the variants, ROMForge shows all of them rather than guessing.")
                }
